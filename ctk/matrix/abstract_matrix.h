 ...
 *
 * AbstractMatrix provides a type-safe, modern C++ interface for matrix
 * operations. It serves as a base class for specialized matrix types (numeric,
 * binary, image).
 *
...
protected:
     ...
     *          before calling Create() (e.g., CV_64F for double, CV_8UC1 for
     * uchar).
     */
    int type = -1;
...
public:
     ...
     */
    AbstractMatrix(const cv::Mat& d) : type(d.type()), ch_size(d.channels()), data(d.clone()) {}
     ...
     * @note Accepts any contiguous container (vector, array, C-array) via
     * implicit conversion.
     */
     ...
     */
    virtual T Get(int x, int y) const { return data.at<T>(y, x); }
     ...
     */
    virtual void Set(int x, int y, T v) { data.at<T>(y, x) = v; }
     ...
     */
    T* begin() noexcept { return data.ptr<T>(0); }

    /**
     * @brief Returns a const iterator to the beginning.
     * @return Const pointer to the first element.
     */
    const T* begin() const noexcept { return data.ptr<T>(0); }

    /**
     * @brief Returns an iterator to the end.
     * @return Pointer to one past the last element.
     */
    T* end() noexcept { return data.ptr<T>(0) + (data.rows * data.cols); }

    /**
     * @brief Returns a const iterator to the end.
     * @return Const pointer to one past the last element.
     */
    const T* end() const noexcept { return data.ptr<T>(0) + (data.rows * data.cols); }
     ...
     */
    bool operator!=(const AbstractMatrix<T>& that) const { return !(*this == that); }
